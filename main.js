...
const priceOfItemsTitle = items.map(function (item) {
...
document.querySelector("#answer4").innerHTML = `${woodItem}`
...
const numOfMaterials = items.filter (function(item){
    if(item.materials >= 8){
        return `<li> ${item.title} ${item.materials} ${item.materials.length} </li>`

    }
}) .join("") 
document.querySelector("#answer5").innerHTML = `${numOfMaterials}`
...
document.querySelector("#answer6").innerHTML = `${sellers}`
